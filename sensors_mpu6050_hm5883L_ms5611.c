 ...
 */
 ...
 #define SENSORS_ENABLE_GPS_ATGM336H
...
static xQueueHandle gpsDataQueue;
STATIC_MEM_QUEUE_ALLOC(gpsDataQueue, 1, sizeof(sensorData.gps));
...
static bool isGpsPresent = false;
#ifdef SENSORS_ENABLE_GPS_ATGM336H
static bool isGpsTestPassed = false; 
#endif
...
static void processGpsMeasurements(void)
{
#ifdef SENSORS_ENABLE_GPS_ATGM336H
    gps_data_t gpsRaw;
    if (isGpsPresent && gps_get_data(&gpsRaw)) {
        sensorData.gps.latitude = gpsRaw.latitude;
        sensorData.gps.longitude = gpsRaw.longitude;
        sensorData.gps.altitude = gpsRaw.altitude;
        sensorData.gps.speed = gpsRaw.speed;
        sensorData.gps.course = gpsRaw.course;
        sensorData.gps.numSat = gpsRaw.sats;
        
        
        sensorData.gps.fix = (gpsRaw.fix && gpsRaw.sats > 6);

        
        xQueueOverwrite(gpsDataQueue, &sensorData.gps);
    }
#endif
}
static void sensorsTask(void *param)
{
    systemWaitStart(); // Ch? l?nh t? app ði?u khi?n
    DEBUG_PRINTD("xTaskCreate sensorsTask IN");
    sensorsSetupSlaveRead(); 
    DEBUG_PRINTD("xTaskCreate sensorsTask SetupSlave done");

    while (1) {
        /* Ch? ng?t t? MPU6050 (T?n s? cao) */
        if (pdTRUE == xSemaphoreTake(sensorsDataReady, portMAX_DELAY)) {
            sensorData.interruptTimestamp = imuIntTimestamp;
            uint8_t intStatus;
            i2cdevReadByte(I2C0_DEV, MPU6050_ADDRESS_AD0_LOW, MPU6050_RA_INT_STATUS, &intStatus);

            /* Bý?c 1: Ð?c d? li?u thô t? I2C */
            ...
            /* Bý?c 2: X? l? tính toán (IMU, Mag, Baro, GPS) */
            ...
            if (isBarometerPresent) {
                processBarometerMeasurements(&(buffer[isMagnetometerPresent ? SENSORS_MPU6050_BUFF_LEN + SENSORS_MAG_BUFF_LEN : SENSORS_MPU6050_BUFF_LEN]));
                      ...
                      /* --- KH?I MONITORING 1HZ (DÀNH CHO TEST) --- */
            /*static uint32_t last_print_time = 0;
            uint32_t now = esp_log_timestamp(); 
            
            if (now - last_print_time >= 1000) { 
                printf("\n--- DRONE MONITOR (1Hz) ---\n");
                printf("[IMU]  Acc:%.2f,%.2f,%.2f | Gyro:%.2f,%.2f,%.2f\n", 
                        sensorData.acc.x, sensorData.acc.y, sensorData.acc.z,
                        sensorData.gyro.x, sensorData.gyro.y, sensorData.gyro.z);
                
                if (isBarometerPresent)
                    printf("[BARO] Press:%.2f hPa | Alt:%.2f m\n", sensorData.baro.pressure, sensorData.baro.asl);

                      printf("[GPS]  Sat:%d | Fix:%s | Lat:%.6f | Lon:%.6f\n",
                        sensorData.gps.numSat, sensorData.gps.fix ? "YES" : "NO",
                        sensorData.gps.latitude, sensorData.gps.longitude);
                 if (isMagnetometerPresent) 
                      printf("[MAG]  X:%.2f | Y:%.2f | Z:%.2f (Gauss)\n", 
            ...
            /* Bý?c 3: Ð?y d? li?u vào Queue ð? bay */
            xQueueOverwrite(accelerometerDataQueue, &sensorData.acc);
            xQueueOverwrite(gyroDataQueue, &sensorData.gyro);
            if (isMagnetometerPresent) xQueueOverwrite(magnetometerDataQueue, &sensorData.mag);
            if (isBarometerPresent) xQueueOverwrite(barometerDataQueue, &sensorData.baro);

            xSemaphoreGive(dataReady); // M? khóa cho Task Stabilizer (?n ð?nh bay)
        }
    }
}
...
{
    /*  Note the ordering to correct the rotated 90Âº IMU coordinate system */
...
#endif
#ifdef SENSORS_ENABLE_GPS_ATGM336H
    // Kh?i t?o GPS v?i UART2, TX=17, RX=16
    gps_init(UART_NUM_2, 17, 16);
    isGpsPresent = true;
    isGpsTestPassed = true; // Ðánh d?u ð? Param IMU hi?n OK
    DEBUG_PRINTI("GPS ATGM336H: Initialized [OK]\n");
#endif
 /*
...
static void sensorsTaskInit(void)
{
  ...
  gpsDataQueue = STATIC_MEM_QUEUE_CREATE(gpsDataQueue);
...
PARAM_ADD(PARAM_UINT8 | PARAM_RONLY, MS5611, &isBarometerPresent) // TODO: Rename MS5611 to LPS25H. Client needs to be updated at the same time.
#ifdef SENSORS_ENABLE_GPS_ATGM336H
PARAM_ADD(PARAM_UINT8 | PARAM_RONLY, gps, &isGpsTestPassed)
#endif
